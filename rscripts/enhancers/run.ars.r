...
reg = new.env()
infile = '../../rawdata/enhancers/rdata/enhancer_coef_elastic0.5_100kb_asinh0.2_cv0.2_H3K27AC.RData' 
load(infile, reg)
outdir = '../../rawdata/enhancers/rdata'
...
ars.score.perm = foreach(n = 1:nperm, .combine = 'append') %dopar% {
...
ggsave(file.path(plotdir, paste(outpref, '_hitsPerIndiv.png', sep = '')), p, width = 6.5, height = 5.6)
...
p = ggplot(sum.sign) + geom_bar(aes(x = n, y = hits), stat = 'identity') + 
  ...
  theme(axis.text.x = element_text(size = 12), axis.title.x = element_text(size = 15), 
        axis.text.y = element_text(size = 12), axis.title.y = element_text(size = 15))
ggsave(file.path(plotdir, paste(outpref, '_indivPerHit.png', sep = '')), p, width = 6.5, height = 5.6)
...
ggsave(file.path(plotdir, paste(outpref, '_normARSdens.png', sep = '')), p, width = 9.75, height = 8.4)
