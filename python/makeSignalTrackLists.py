...
fragfile = open(args.outpref + '.fraglen', 'w') # File of fragment lengths, columns 1. input_bam 2. frag_len
...
combfile2 = open(args.outpref + '_merged_combrep.tab', 'w') # Same as previous one but with merged inputs
...
with open(args.qcfile, 'r') as infile:
    for line in infile:
        ...
        if not filename in paired:
            ...
            continue
        frag = fields[2].split(',')[0]
        fragfile.write(filename + '\t' + frag + '\n')
...
for pref, fs in repmap.iteritems():
    inputfile = inputmap[pref]
    indiv = pref.split('_')[2]
    mark = pref.split('_')[3]
    if not indiv in segmap:
        print >> sys.stderr, indiv, 'is missing from the genders file.'
        continue
    combfile.write('\t'.join([';'.join(fs), inputfile, pref]) + '\n')
    combfile2.write('\t'.join([';'.join(fs), all_inputs, pref]) + '\n')
    ...
    namefile.write('\t'.join([nameStart, re.sub('.bam', '', inputfile), indiv, mark]) + '\n')
    for f in fs:
